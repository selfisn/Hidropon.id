 ...
 <div class="invoice p-3 mb-3">
                  ...
                  <table class="table table-striped">
                    ...
                    <tbody>
                      ...
                      foreach ($this->cart->contents() as $items) {
                        ...
                        $berat_produk = $items['qty'] * $produk->berat_produk;

                        $t_berat = $t_berat + $berat_produk;
                      ?>
                        <tr>
                          ...
                          <td class="text-center"><?= $berat_produk ?> gr</td>
                        </tr>
                      <?php  } ?>
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <tr>
                        ...
                        <!-- type="text" ganti ke type="hidden jika ingin disembunyikan" -->
                        <input name="ongkir" type="hidden" id="ongkosnya" value="">
                        <input name="total_bayar" type="hidden" id="totalnya" value="">
                      </tr>
                    </table>
                  </div>
                </div>
              ...
              <input name="estimasi" >
              <input name="berat" value="<?= $t_berat ?>"hidden><br>
            ...
            <script>
              $(document).ready(function() {
                ...
                $("select[name=expedisi]").on("change", function() {
                  ...
                  //mendapatkan id_kota tujuan terpilih
                  var id_kota_tujuan_terpilih = $("option:selected", "select[name=kota]").attr('id_kota');
                  //mengambil data ongkos kirim
                  var total_berat = <?= $t_berat ?>;
                ...
                $('#paket').change(function() {
                  ...
                  var grand = <?= $this->cart->total(); ?>;
                  var total = parseInt(grand) + parseInt(textArray[2]);
                  $("#ongkir").html("Rp. " + Number(textArray[2]).toLocaleString());
                  $("#total_bayar").html(total.toLocaleString());
                  $("#ongkosnya").val(textArray[2]);
                  $("#totalnya").val(total);


                  //estimasi dan ongkir
                  var estimasi = $("option:selected", this).attr('estimasi');
                  $("input[name=estimasi]").varl(estimasi); 
                  //$("input[name=ongkir]").varl(ongkir); 
                  $("input[name=total_bayar]").varl(data_total_bayar);
                });
                // $("select [name=paket]").on("change", function() {
                //   //menampilkan ongkir
                //   var dataongkir = $("option:selected", this).attr('ongkir');
                //   //var reverse = dataongkir.toString().split('').reverse().join('',)
                //   // ribbuan_ongkir = reverse.match(/\d{1,3}/g);
                ...
                //   ribbuan_total_bayar = reverse2.match(/\d{1,3}/g);
